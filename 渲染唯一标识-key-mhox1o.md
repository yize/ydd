...
**路径：**自定义页面随意点击一个组件 >> 右侧的高级 >> 是否渲染 >> 渲染唯一标识
该配置项一般配合「是否渲染」和「循环」功能使用
...
配置方式如下：增加变量数据源：hobby「最喜欢的游戏」表单标识设置为 game，「是否渲染」绑定变量「state.hobby === '游戏'」

「最喜欢的运动」表单标识设置为 sport，「是否渲染」绑定变量「state.hobby === '运动'」「爱好」设置 onChange 动作
export function onChagne({ value, actionType, item }) {  this.setState({  	hobby: value,  });}

「提交」按钮绑定 onClick 动作
export function submit() {	console.log(this.$('form').getValue());}
